...
vp2.as.oce <- function(data, file, directory) {
  ...
  serialnumber <- gsub(
    ".*=", "\\1",
    grep("serialnumber", lines, ignore.case = TRUE, value = TRUE)
  )
  lines <- lines[
    (which(lines == "[COLUMNS]") + 1):(which(lines == "[DATA]") - 2)
    ]
  ...
  getunit <- function(var) {
    ...
    if (!identical(grep(";;", unit), integer(0))) {
      unit <- "unknown/undefined"
    }
    if (identical(variable, character(0))) {
      unit <- "unknown/undefined"
    }
    return(unit)
  }
  ...
  donecols <- c(
    "salinity", "temperature", "pressure", "conductivity", "time",
    "filename", "lat", "lon"
  )
