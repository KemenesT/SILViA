...
test_that("vp2_as_oce outputs an object with all the expected components", {
  ...
  units <- c("PSU", "DegC", "dBar", "mS/cm", "",
             "", "m", "Ms-1", "kg/M3", "ug/l")

  names <- c("scan", "salinity", "temperature", "pressure", "conductivity",
             "time", "latitude", "longitude", "date", "depth", "velocity",
             "density", "chlorophyll")
  ...
  units <- c("PSU", "DegC", "dBar", "mS/cm", "",
             "", "m", "Ms-1", "kg/M3", "ntu", "btu", "ntu")

  names <- c("scan", "salinity", "temperature", "pressure", "conductivity",
             "time", "latitude", "longitude", "date", "depth", "velocity",
             "density", "Nephelometer", "OBS", "turbidity")
...
test_that("vp2_as_oce gives an error message for an unspecified CTD type", {
  on.exit(unlink(paste0(
    tempdir(), "\\",
    list.files(tempdir(), pattern = ".vp2")
  )))
  ...
  expect_error(vp2.as.oce(data = cast, file = file_name, directory = tempdir(),
                          type = "potato"),
                label = "'type' must be one of 'Chlorophyll' or 'Turbidity'")
})
